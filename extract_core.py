...
def _fetch_xano_pages(endpoint):
    all_rows = []
    page = 1
    while True:
        resp = requests.get(endpoint, params={"page": page, "per_page": 100}, timeout=15)
...
def run_extraction(start_row: int, end_row: int | None):
    ...
    try:
        existing     = _fetch_xano_pages(summary_endpoint)
    ...
    try:
        all_rows = _fetch_xano_pages(get_endpoint)
